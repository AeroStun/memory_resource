...
# include "default_resource.h"
  ...
  struct aligned_size
  {
    ...
    constexpr
        ...
        : value(sz | (std::bit_width(align) - 1u))
    {
  ...
  private:
    static constexpr unsigned _S_size_digits
        = (numeric_limits<size_type>::digits
           + std::bit_width(bits_per_word) - 1) / 2;
  ...
  struct chunk : bitset
  {
    ...
    { assert(bytes <= chunk::max_bytes_per_chunk()); }
  ...
  struct big_block
  {
    ...
    static constexpr size_t min = bit_ceil(static_cast<std::size_t>(numeric_limits<size_t>::digits));
  ...
  struct __pool_resource::_Pool
  {
    ...
    void replenish(memory_resource* __r, const pool_options& __opts)
    {
      ...
      size_t __alignment = std::bit_ceil(__block_size);
    ...
    {
      const size_t __alignment = std::bit_ceil(block_size());
    ...
    struct vector
    {
      ...
      iterator insert(chunk&& c, memory_resource* r)
      {
        ...
        assert(std::is_sorted(begin(), back));
        ++size;
        return back;
      }
                              ...
                              size_t alignment [[maybe_unused]])
  {
    ...
    assert(it != _M_unpooled.end() && it->pointer == p);
    if (it != _M_unpooled.end() && it->pointer == p) // [[likely]]
    {
      const auto b = *it;
      assert(b.size() == b.alloc_size(bytes));
      assert(b.align() == alignment);
  ...
  {
    if (int err = ::tss_create(&_M_key, destroy_TPools))
  ...
  {
    release();
    ::tss_delete(_M_key); // does not run destroy_TPools
  }
  ...
  synchronized_pool_resource::release()
  {
    ...
    {
      ::tss_delete(_M_key); // does not run destroy_TPools
      ::tss_create(&_M_key, destroy_TPools);
  ...
  {
    __pool_resource::_Pool* pools = nullptr;
    if (auto tp = static_cast<_TPools*>(::tss_get(_M_key)))
    {
      pools = tp->pools;
      // assert(tp->pools);
    }
      ...
      -> _TPools*
  {
    ...
    try
    {
      ...
      constructed = true;
      // assert(::tss_get(_M_key) == nullptr);
      if (int err = ::tss_set(_M_key, p))
        throw system_error{err, generic_category()};;
    }
    catch(...)
    {
